...
def main():
    ...
    checkpoint_prefix = ''
    # checkpoint_prefix = './gcjpy/model.bin'
    ...
    model_sub = RobertaModel.from_pretrained('').to(args.device)
   ...
   # tokenizer_mlm = RobertaTokenizer.from_pretrained(args.base_model)
    ...
    attacker = CrossDomainAttacker(args, model_sub, model, tokenizer)
    ...
    success_records = []
    # 清空文件
    open("successful_attacks.jsonl", 'w').close()
    for index, example in enumerate(eval_dataset):
        example_start_time = time.time()
        code = source_codes[index]
        sub = subs[index]
        sub_2 = subs_2[index]
        idx_now = idx_group[index]
        #打印clean样本
        # print("clean code:\n", code)
        ...
        if is_success == 1:
            ...
            record = {
                "idx":idx_now,
                "orig_code":code,
                "adv_code":adv_code_save
            }
            with open('./successful_attacks.jsonl', 'a', encoding="utf-8") as f:
                f.write(json.dumps(record) + "\n")
                print("成功写入攻击成功样本，idx为", idx_now)
