...
# 假设你要加的一些参数，比如 batch_size 和 lr（可以换成你需要的）
global_processes=5
global_cur_process=4
global_SeedsCount=15
global_mutationCount=8
global_num_generation=10
global_population_size=6


start_from_break_point=True
if start_from_break_point:
    print("从断点继续运行程序")
    global_count_succeed_attack_all=54  # 记录所有生成的攻击样例的数目
    global_count_processed=120  # 记录已经处理的代码段，为的是统计当前程序的运行进度
    global_count_GA_ATTACK=0 # 统计基于遗传算法攻击成功的次数
    global_count_GA_ATTACK_deep_search=16  # 统计基于遗传算法的深度搜索成功的次数
    global_noq=11519 # 记录已经处理的代码段，为的是统计当前程序的运行进度
    break_point =  188583 # 记录从哪个代码段开始处理
else:
    print("从头开始运行程序")
    global_count_succeed_attack_all=0  # 记录所有生成的攻击样例的数目
    global_count_processed=0  # 记录已经处理的代码段，为的是统计当前程序的运行进度
    global_count_GA_ATTACK=0  # 统计基于遗传算法攻击成功的次数
    global_count_GA_ATTACK_deep_search=0  # 统计基于遗传算法的深度搜索成功的次数
    global_noq=0  # 记录已经处理的代码段，为的是统计当前程序的运行进度
    break_point = -1  # 记录从哪个代码段开始处理


global_memo='test_Unixcode_nonvulgen_all'

# 构造日志文件名
log_name = f"{script_name}_pro_{global_processes}_cur_{global_cur_process}_memo_{global_memo}.log"
log_path = os.path.join(log_dir, log_name)

if not start_from_break_point:
    # 如果存在旧日志，就删除
    if os.path.exists(log_path):  # 检查文件是否存在
        os.remove(log_path)  # 如果文件存在，则删除该文件
        print(f"文件 {log_path} 已删除")
...
def genetic_algorithm(code,
    ...
    for generation in range(num_generations):
        ...
        for chromosome in population:
            data, checkresult, certaintyValue,error_flag = fitness(chromosome,
                                                        code=code,
                                                        target=target,
                                                        code_idx=code_idx,
                                                        noq_flag=True)
...
def fitness(vector,code,target,code_idx,noq_flag=False):
    '''
    # 定义适应度函数
    '''
    # 要按照txl命令行的要求把源代码段转换为C语言代码段，否则无法处理。
    # logger.info("Fitness function中，传递过来的vector为："+str(vector))
    TRANSFORMCODE = ""+str(global_cur_process)+".c"
    ...
    for index in range(1, len(vector) + 1):
        if vector[index - 1] == 0:
            ...
            ACTION = index
            # 取出转换后的结果
            content = TransformCodePreservingSemantic(TRANSFORMCODE=TRANSFORMCODE, ATCION=ACTION,code=code,cur_process=global_cur_process)
            # 添加考慮輸出為空的異常情況
            if content == None:
                continue
    ...
    # logger.info("Fitness function中，最终转换后的代码为："+str(content))
    if content == None:
    ...
    # 否则无法处理该文件
    check_jsonl_path = ""
    with open(check_jsonl_path, 'w') as f:
        data = {}
        data['project'] = 'test'
        data['commit_id'] = 'test'
        data['target'] = target
    ...
    results, error_flag = run_robustness_attackV2.main(test_data_file=check_jsonl_path,testing=True, isGenerateTestSuccessSamples=False, isRevalidate=False,target_model_path=target_model_path)
    if noq_flag:
        global noq
        noq+=1
    ...
    return data,checkresult,certaintyValue,error_flag
...
def iterateMutation(test_file_path = 'test_success_examples.jsonl',
                    ...
                    SeedsCount=8,#这是初始种子数量
                    num_generations=10, #这是演化的代数
                    mutationCount=8,#这是设置扰动染色体的多少位。
                    ):
    attack_succeed_results_path = ""+str(processes)+"_"+str(cur_process)+"_"+str(global_memo)+".jsonl"  # 这是最终存储攻击成功的样例文件
    ...
    logger.info(
        ...
        f"SeedsCount={SeedsCount}, num_generations={num_generations}, mutationCount={mutationCount}")
    # 记录所有生成的攻击样例的数目
    if not start_from_break_point:
        if os.path.exists(attack_succeed_results_path):
            os.remove(attack_succeed_results_path)   # 删除文件
            # print(f"文件 {attack_succeed_results_path}存在 已删除")
            logger.info(f"文件 {attack_succeed_results_path}存在 已删除")
    ...
    count_succeed_attack_all=global_count_succeed_attack_all
    count_processed=global_count_processed
    # 统计基于遗传算法攻击成功的次数
    count_GA_ATTACK = global_count_GA_ATTACK
    # 统计基于遗传算法的深度搜索成功的次数
    count_GA_ATTACK_deep_search = global_count_GA_ATTACK_deep_search
    ...
    for idx, js in enumerate(js_all):
        if int(js['idx']) == break_point or break_point==-1:
            # logger.info("从断点位置继续运行，断点位置是："+str(break_point))
            start_run = True
        if start_run:
            ...
            # if js['target']==1:
                ...
                while is_Continue:
                    ...
                    while True:
                        ...
                        data,checkresult,certaintyValue,error_flag=fitness(vector=vector,
                                                                code=code_segments,
                                                                target=js['target'],
                                                                code_idx=code_idx,
                                                                noq_flag=True)
                        ...
                        if data != None and checkresult != None and certaintyValue!=None:
                            ...
                            if key > 0.5:
                                ...
                                break
                            if index_random >= SeedsCount:
                                break
                        elif error_flag!=None:
                            logger.info("len(logits)==0")
                        else:
                            logger.info("返回的数据为空，要小心了")
                            logger.info("当前的随机vector为： "+str(vector)+
                                        "data： "+str(data)+
                                        ",checkresult"+str(checkresult)+
                                        ",certaintyValue"+str(certaintyValue)+
                    ...
                    if best_data!=None and best_fitness>0.5:
                        ...
                        with open(attack_succeed_results_path, 'a') as file:
                            file.write(json.dumps(best_data, ensure_ascii=False) + "\n")
...
def main():
    ...
    if PROJECT_NAME in parent_parent_dir_name:
        ...
        global noq
        noq=global_noq
        iterateMutation(cur_process=global_cur_process,
